...
def _write_capture(d):
    (d / "meta.json").write_text(json.dumps({
        "app_version": "0.0.1", "calculator": {"model": "n0200", "first_boot": True}}))
    usb = [
        {"t": 0.0, "dir": "out", "bmRequestType": C.REQ_OUT, "bRequest": C.DFU_DNLOAD,
         "wValue": 0, "wIndex": 0, "len": 5, "data": "2100000020"},          # SET_ADDRESS 0x20000000
        {"t": 0.1, "dir": "in", "bmRequestType": C.REQ_IN, "bRequest": C.DFU_UPLOAD,
         "wValue": 2, "wIndex": 0, "len": len(SERIAL), "data": SERIAL.encode().hex()},  # identity read
        {"t": 0.2, "dir": "in", "bmRequestType": C.REQ_IN, "bRequest": C.DFU_GETSTATUS,
         "wValue": 0, "wIndex": 0, "len": 6, "data": "000000000200"},        # OK / dfuIDLE
    ]
    (d / "usb.jsonl").write_text("\n".join(json.dumps(r) for r in usb))
    web = [
        {"method": "POST", "url": "https://my.numworks.com/users/sign_in", "status": 302},
        {"method": "GET", "url": "https://my.numworks.com/firmwares/n0200/stable.json", "status": 200},
        {"method": "POST", "url": "https://my.numworks.com/devices", "status": 403,
         "req_body_b64": _b64json({"serial": SERIAL})},                       # enrollment refused
        {"method": "GET", "url": "https://my.numworks.com/firmwares/n0200/stable.dfu", "status": 200,
         "resp_len": 237606, "resp_sha256": "838d8fe32834"},
    ]
...
def test_write_to_calculator_is_flagged(tmp_path):
    (tmp_path / "usb.jsonl").write_text("\n".join(json.dumps(r) for r in [
        {"dir": "out", "bRequest": C.DFU_DNLOAD, "wValue": 0, "data": "2100000098"},   # SET_ADDRESS
...
def test_har_web_loading(tmp_path):
    har = {"log": {"entries": [
        {"startedDateTime": "2026-07-13T10:00:00Z",
         "request": {"method": "GET", "url": "https://my.numworks.com/firmwares/n0200/stable.dfu",
...
def _run_capture_dict(web_status_manifest=403):
    ...
    return {
        ...
        "usb": {
            "model": "n0200", "family": "scientifique", "os_version": None,
            "slot_info_valid": False,
            "transfers": [
                {"seq": 0, "dir": "OUT", "bmRequestType": "0x21", "bRequest": C.DFU_DNLOAD,
                 "wValue": "0x0000", "wIndex": 0, "data_len": 5, "data": "2100000098"},  # SET_ADDR 0x98000000
                {"seq": 1, "dir": "IN", "bmRequestType": "0xa1", "bRequest": C.DFU_UPLOAD,
                 "wValue": "0x0002", "wIndex": 0, "wLength": 32, "data_len": len(SERIAL),
                 "data": SERIAL.encode().hex()},                                          # identity read
            ],
        },
        "web": {
            "outcome": "refused_or_error", "model": "n0200", "channel": "stable",
            "transfers": [
                {"seq": 0, "request": {"method": "POST", "url": "https://my.numworks.com/users/sign_in",
                                       "headers": {"Cookie": "[REDACTED]"}, "body": {"form": {"user[email]": "x"}}},
                 "response": {"status": 302, "headers": {}, "body": {"size": 0, "sha256": "e3b0", "content_type": "text/html"}}},
                {"seq": 1, "request": {"method": "GET", "url": "https://my.numworks.com/firmwares/n0200/stable.json"},
                 "response": {"status": web_status_manifest, "headers": {},
                              "body": {"size": 60, "sha256": "aa", "content_type": "application/json",
