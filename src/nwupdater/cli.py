...
def main(argv=None) -> int:
    ...
    p_login.add_argument(
        "--token", metavar="VALUE", help="paste the remember_user_token cookie value directly"
    )
    p_login.add_argument(
        "--email", metavar="ADDR", help="built-in login: account email (password prompted)"
    )
    p_login.add_argument(
        "--password",
        metavar="PWD",
        help="password (otherwise prompted without echo) — never stored",
    )
    ...
    p_id.add_argument(
        "--virtual", metavar="MODEL", help="use a virtual device (n0110, n0120, n0200…)"
    )
    ...
    p_cat.add_argument(
        "--fetch", action="store_true", help="refresh the catalog online (my.numworks.com)"
    )
    p_cat.add_argument(
        "--catalog-file", metavar="PATH", help="load the catalog from a local JSON file"
    )
    ...
    p_ins.add_argument(
        "--from-cache", action="store_true", help="flash from the cache (pre-downloaded)"
    )
    ...
    p_app.add_argument(
        "--uninstall", metavar="NAME", help="remove an installed app (compacts the region)"
    )
    p_app.add_argument(
        "--reorder", metavar="A,B,C", help="reorder installed apps (comma-separated names)"
    )
    p_app.add_argument(
        "--yes",
        "-y",
    ...
    p_pre.add_argument(
        "version", nargs="?", help="version to cache (e.g. 25.2.0); omit with --download"
    )
    p_pre.add_argument(
        "--download",
        action="store_true",
    ...
    p_diag = sub.add_parser(
        "diagnose", help="read-only diagnostic + USB capture (hardware harness)"
    )
    p_diag.add_argument(
        "--virtual", metavar="MODEL", help="self-test against a virtual device (n0110…)"
    )
    ...
    p_diag.add_argument(
        "--out", metavar="FILE", help="JSON report path (default: nwupdater-diagnostic-<ts>.json)"
    )
    ...
    p_cap.add_argument(
        "--model",
        metavar="Nxxxx",
        help="server-side target model (default: derived from bcdDevice)",
    )
    p_cap.add_argument(
        "--channel", default="stable", choices=["stable", "beta"], help="firmware channel"
    )
    p_cap.add_argument("--out", metavar="FILE", help="JSON dump path")
    p_cap.set_defaults(func=_cmd_capture)

    p_pair = sub.add_parser(
        "pair", help="pair the calculator to the account (heartbeat POST /devices)"
    )
    ...
    p_scr.add_argument(
        "--no-auto-import", action="store_true", help="do not mark the script as auto-imported"
    )
    p_scr.add_argument(
