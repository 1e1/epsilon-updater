...
_REQ_NAME: dict = {
    ...
    C.DFU_DNLOAD: "DNLOAD",
    C.DFU_UPLOAD: "UPLOAD",
    C.DFU_GETSTATUS: "GETSTATUS",
    C.DFU_CLRSTATUS: "CLRSTATUS",
    C.DFU_GETSTATE: "GETSTATE",
    C.DFU_ABORT: "ABORT",
}
...
def _norm_web_transfer(t: dict) -> dict:
    ...
    return {
        ...
        "req_body_text": json.dumps(req.get("body"), ensure_ascii=False)
        if req.get("body")
        else None,
    }
...
def _norm_hook_web(r: dict) -> dict:
    return {
        ...
        "resp_sha256": r.get("resp_sha256"),
        "resp_body_text": r.get("resp_body"),
        "req_body_text": r.get("req_body"),
        "scenario": r.get("scenario"),
    }
...
def _from_hook(obj: dict) -> dict:
    ...
    return {
        ...
        "usb": [
            _norm_hook_usb(r) for r in obj.get("usb", []) if r.get("via", "control") == "control"
        ],
...
def load_capture(source: str | Path | dict) -> dict:
    ...
    if isinstance(source, dict):
        ...
        jf = (
            ...
            else (p / "capture.json" if p.is_dir() else None)
        )
...
def api_map_by_scenario(cap: dict) -> dict:
    ...
    for row in cap["web"]:
        ...
        e = d["endpoints"].setdefault(
            key, {"method": key[0], "endpoint": key[1], "count": 0, "statuses": []}
        )
        e["count"] += 1
    ...
    return {
        sc: {"web": list(d["endpoints"].values()), "usb_transfers": d["usb_transfers"]}
...
def analyze_web(records: list[dict]) -> dict:
    ...
    for row in records:
        ...
        if kind == "enrollment?":
            enrollments.append(
                {"method": method, "url": url, "status": status, "body": _body_text(row)[:2000]}
            )
    ...
    return {
        "calls": calls,
        "refusals": refusals,
        "enrollment_candidates": enrollments,
...
def analyze(dump_dir: str | Path) -> dict:
    ...
    if cap["usb"]:
        if usb.writes:
            findings.append(
                f"⚠️ {len(usb.writes)} WRITE(S) to the calculator detected "
        ...
        if usb.reads:
            ...
            findings.append(
                f"identity read over USB: {span} — addresses {[hex(a) for a, _ in usb.reads][:6]}."
            )
...
def format_report(rep: dict) -> str:
    ...
    if rep["meta"]:
        m = rep["meta"]
        L.append(
            f"context: {m.get('calculator', {}).get('model', '?')} "
