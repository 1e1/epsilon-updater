...
def diff_bytes(address: int, a: bytes, b: bytes) -> SegmentDiff:
    ...
    return SegmentDiff(
        address=address, len_a=len(a), len_b=len(b), compared=n,
        equal_bytes=equal, equal_ratio=(equal / n if n else 0.0),
        equal_ratio_after_prefix=(equal_after / tail if tail else 0.0),
        common_prefix=prefix, longest_equal_run=_longest_equal_run(a, b),
        equal_blocks16=eq_blocks, total_blocks16=blocks,
...
class DfuDiff:
    ...
    def to_dict(self) -> dict:
        return {
            "size_a": self.size_a, "size_b": self.size_b,
            "only_in_a": [f"0x{a:08x}" for a in self.only_in_a],
            "only_in_b": [f"0x{a:08x}" for a in self.only_in_b],
            "segments": [{**asdict(s), "address": f"0x{s.address:08x}",
                          "verdict": classify(s)} for s in self.segments],
        }
...
def diff_images(a: FirmwareImage, b: FirmwareImage) -> DfuDiff:
    ...
    return DfuDiff(
        size_a=a.total_size, size_b=b.total_size, segments=seg_diffs,
