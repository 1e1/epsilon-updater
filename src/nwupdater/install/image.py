...
class FirmwareImage:
    ...
    def synthetic(
        ...
        buf[userland_off : userland_off + C.USERLAND_HEADER_SIZE] = headers.pack_userland_header(
            version,
            storage_addr_ram=mem.sram_origin + 0x1000,
            storage_size_ram=0x10000,
            external_apps_flash=apps,
        )
        return cls(
            [FirmwareSegment(base, bytes(buf))],
            version=version,
    ...
    def to_dfuse(
        self,
        *,
        id_vendor: int = C.USB_VID,
        id_product: int = C.PID_EPSILON,
        bcd_device: int | None = None,
    ) -> bytes:
        ...
        target_block = (
            struct.pack("<6sBI255sII", b"Target", 0, 1, name, len(target), len(self.segments))
            + target
        )
        body = struct.pack("<5sBIB", b"DfuSe", 1, 11 + len(target_block) + 16, 1) + target_block
        suffix_wo_crc = body + struct.pack(
            "<HHHH3sB", bcd_device, id_product, id_vendor, 0x011A, b"UFD", 16
        )
